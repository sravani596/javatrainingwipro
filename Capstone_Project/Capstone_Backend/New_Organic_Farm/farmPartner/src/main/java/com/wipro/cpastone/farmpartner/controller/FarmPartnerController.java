...
//	}
    ...
    @PostMapping("/login")
    public ResponseEntity<FarmPartner> login(@RequestBody Map<String, String> loginData) {
        String email = loginData.get("email");
        String password = loginData.get("password");

        FarmPartner farmPartner = farmPartnerService.login(email, password);

        if (farmPartner != null) {
            return ResponseEntity.ok(farmPartner);
        } else {
